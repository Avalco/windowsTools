...
public final class JIntellitype implements JIntellitypeConstants {
   ...
   private static void loadLibrary() {
       ...
       File extractedLibrary = Paths.get(configDir.getAbsolutePath(), "winTools", "1.0.0",
           libraryName)
           .toFile();
       try {
           ...
           try {
               fromJarToFs("lib/" + libraryName, extractedLibrary.getAbsolutePath());
               System.load(extractedLibrary.getAbsolutePath());
           } catch (Throwable exAllFailed) {
              exAllFailed.printStackTrace();
